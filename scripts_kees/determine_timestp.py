...
# prefer an interactive backend when available; otherwise fall back to Agg (headless)
try:
    mpl.use('Qt5Agg')   # needs PyQt5 / PySide2
except Exception:
    try:
        mpl.use('TkAgg')   # needs tkinter
    except Exception:
        mpl.use('Agg')     # headless fallback
...
RUN_ROOT = Path(r'd:\Git\thermo-morphological-model\runs\20250825_determin_stability\20250828_batch')
n_runs   = 18
...
def main():
    runs = [RUN_ROOT / f'run{idx:02d}' for idx in range(1, 36)]
    data = {}
    for r in runs:
    ...
    ref = data.get('run01')
    if ref is None:
        print("Reference not available. Aborting.")
        return

    labels, rmses, biases, sis = [], [], [], []
    for idx in range(1, n_runs):
        name = f'run{idx:02d}'
        arr = data.get(name)
        labels.append(name)
        if arr is None:
        ...
        print(f"{name} vs reference -> RMSE: {rmse:.6f}, Bias: {bias:.6f}, SI: {si:.3f}%")
    ...
    ax.set_title('Comparison vs reference: ground_temperature_distribution')
    ax.legend()
    fig.tight_layout()
